...
export default function Posts({posts, data}) {
  ...
  const submit = (event) => {
    ...
    axios.post('/api/postUpdate', null, {
        params: {
  ...
  return (          
    ...
    <section className="ContentBox">
      ...
      {data.user ? 
        (<button onClick={modify}>수정</button>) : (<span></span>)
      }
    ...
    return (
      ...
      <form>
          <div className='category'>
              <select ref={categoryReference} defaultValue={posts[i].category} id='category'>
          ...
          <div className='title'>
              <input ref={titleReference} defaultValue={posts[i].title} type='text' id='title' placeholder='제목'></input>
          </div>
          <div className='content'>
              <textarea ref={contentReference} defaultValue={posts[i].content} id='content'></textarea>
          </div>
          <Link href='/postList'>
